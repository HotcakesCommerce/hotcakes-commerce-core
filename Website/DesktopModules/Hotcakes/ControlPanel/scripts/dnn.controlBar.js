...
dnn.controlBar.init = function (settings) {
    ...
    if (settings.bookmarkedModuleKeys)
    ...
    var toggleModulePane = function (pane, show) {
        ...
        if (show) {
            if (!paneVisible) {
                pane.animate({ height: 'show' }, 100, shiftFormPane);
            }

        } else {
            if (paneVisible) {
                pane.animate({ height: 'hide' }, 100, shiftFormPane);
            }
        }
    };

    var hidePerformancePane = function () {
        var viewMore = $(".hcProductPerformanceBar, .hcCategoryPerformanceBar").find(".hcViewMore");
        viewMore.attr("data-state", "hidden");
